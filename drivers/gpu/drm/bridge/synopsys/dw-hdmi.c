...
static void dw_hdmi_i2c_init(struct dw_hdmi *hdmi)
{
	...
	if (hdmi->plat_data->dev_type == RCAR_HDMI)
		hdmi_modb(hdmi, 0x00, HDMI_I2CM_DIV_FAST_STD_MODE_MASK, HDMI_I2CM_DIV);
	else
...
{
	extern struct drm_display_mode *drm_display_mode_from_vic_index(
		struct drm_connector *connector, const u8 *video_db, u8 video_len, u8 video_index);
	u8 def_modes[] = { 108 };
	...
	if (edid) {
		...
		mode = drm_display_mode_from_vic_index(connector, def_modes, 31, 0);
		if (mode) {
			printk("(%s %d) 0==drm_get_edid, try drm_display_mode_from_vic_index() ...\n", __FILE__, __LINE__);
			mode->type = DRM_MODE_TYPE_PREFERRED;
			drm_mode_probed_add(connector, mode);
			ret++;
		} else {
			printk("(%s %d) 0==drm_get_, drm_display_mode_from_vic_index() NG\n", __FILE__, __LINE__);
		}
...
static int __drm_helper_probe_single_connector_modes(struct drm_connector *connector, uint32_t maxX, uint32_t maxY)
{
	return drm_helper_probe_single_connector_modes(connector, 1280, 720);
}
...
static const struct drm_connector_funcs dw_hdmi_connector_funcs = {
	//.fill_modes = __drm_helper_probe_single_connector_modes,
		...
		const struct dw_hdmi_plat_data *plat_data)
{
	...
	if (0 == plat_data->regm) {
		...
		case 4:
			...
			hdmi->reg_shift = 2;
			printk("reg-io-width=4\n");
			break;
		case 1:
			printk("reg-io-width=1\n");
		...
		printk("(%s %d) drm: 0 != plat_data->regm\n", __FILE__, __LINE__);
